...
export async function startNode() {
        ...
        switch (messageJSON.type) {
            ...
            case 'start-election':
                ...
                if (!lastBlock) {
                    ...
                    const messageSelectedGeneratorEncoded = new TextEncoder().encode(messageSelectedGenerator);
            ...
            case 'generator':
                ...
                const messageNewBlockEncoded = new TextEncoder().encode(messageNewBlock);
            ...
            case 'new-local-tx':
                ...
                const newTx = new Transaction(timestamp, sender, receiver);
                ...
                const messageTxEncoded = new TextEncoder().encode(messageTx);
                await node.services.pubsub.publish(TOPIC, messageTxEncoded);
                break;
            case 'new-tx':
                const tx = new Transaction(messageJSON.data.tx.timestamp, messageJSON.data.tx.sender, messageJSON.data.tx.receiver);
            ...
            case 'new-local-candidate':
                ...
                const messageCandidateEncode = new TextEncoder().encode(messageCandidate);
    ...
    function selectGeneratorBasedOnSeed(seed, pingNode) {
        ...
        function sortedPeersInPubSub(peers, pingNodeId) {
            const peersToArrayOfString = peers.map((peer) => {
        ...
        function chooseRandomNode(peers) {
